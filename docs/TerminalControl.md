...
- **Output as log** - The output pane is a read-only **multiline `TextBox`** (not `RichTextBox`) so Windows’ RichEdit control does not draw its own insertion caret over the stream. It uses no tab stop, `HideSelection` when unfocused, horizontal and vertical scroll bars, `WordWrap = false`, and `WM_VSCROLL` / `SB_BOTTOM` after each append to follow the tail of the log.
