...
- `TerminalOutputRichTextBox.cs` - Output `RichTextBox` subclass: non-selectable for keyboard focus, `EM_HIDESELECTION`, and `HideCaret` only while the log actually has focus (avoids hiding the thread caret for the command `TextBox`). `FocusCommandInput` calls `ShowCaret` on the command line so the insertion caret stays visible when typing.
